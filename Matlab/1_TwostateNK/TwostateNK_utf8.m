...
figure;
subplot(231);
plot(xvec,rvec0*4,'k*-','LineWidth',2.0);
hold on;
plot(xvec,rvec1*4,'k*--','LineWidth',2.0);
plot(xvec,[0 0],'r-');
title('政策金利, r_{n}','FontWeight','Normal');
xticks([0 1]);
xticklabels({'H','L'});
set(gca,'Fontsize',12);
...
plot(xvec,[0 0],'r-');
title('産出ギャップ, y','FontWeight','Normal');
xticks([0 1]);
xticklabels({'H','L'});
set(gca,'Fontsize',12);
...
hold on;
plot(xvec,pvec1*4,'k*--','LineWidth',2.0);
plot(xvec,[0 0],'r-');
title('インフレ率, \pi','FontWeight','Normal');
xticks([0 1]);
xticklabels({'H','L'});
set(gca,'Fontsize',12);
m = legend('p_H=0','p_H=0.025','Location','SouthWest');
m.FontSize = 8;
saveas(gcf,'TwostateNK','epsc2');